...
# Habilita todos os repositórios oficiais (main/community/testing)
RUN echo "https://dl-cdn.alpinelinux.org/alpine/edge/main"       >> /etc/apk/repositories && \
    echo "https://dl-cdn.alpinelinux.org/alpine/edge/community"  >> /etc/apk/repositories && \
    echo "https://dl-cdn.alpinelinux.org/alpine/edge/testing"    >> /etc/apk/repositories

# 1A) toolchain + libs que SEMPRE existem
RUN apk update && apk add --no-cache \
    ...
    x264-dev libvpx-dev soxr-dev libwebp-dev openjpeg2-dev

# 1B) libs opcionais (presentes em edge/testing)
...
RUN ./configure --prefix=/usr/local --enable-gpl --enable-nonfree --enable-version3 \
